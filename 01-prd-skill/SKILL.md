...
- **Universal Proactive Executive Expert Mandate (UNRESTRICTED Across ALL Phases)**: The agent MUST NOT act as a passive order-taker in ANY phase (Ideation, Vibe Brief, PRD, or Strategy). Proactively conduct web searches (`search_web`) for current live market trends, competitor complaints, ASO keyword volume, platform guidelines (Apple HIG, Android Material 3, WCAG 2.2 accessibility $\ge 44\times44\text{pt}$ touch targets), store compliance (Apple Guideline 5.1.1, GDPR Article 17 self-serve account deletion, legal triad `/privacy`, `/terms`, `/support`), retention loops, and business monetization models.
