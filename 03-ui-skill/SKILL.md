...
15. **Universal Proactive Executive Expert Mandate (UNRESTRICTED UI/UX Proactivity Across ALL Screens)**: DO NOT act as a passive order-taker. The agent MUST proactively conduct live web searches (`search_web`) for current platform guidelines (Apple HIG, Android Material 3, WCAG 2.2 accessibility $\ge 44\times44\text{pt}$ touch targets) and audit/inject universal UX, legal, and navigation controls across EVERY screen type (Welcome, Auth, Dashboard, Feeds, Settings, Forms, Paywalls, Modals, Details):
    - **Legal & App Store Compliance**: Mandatory `Privacy Policy`, `Terms of Service`, `Restore Purchases`, and self-serve `Account Deletion` links (Apple Guideline 5.1.1 & GDPR Article 17).
